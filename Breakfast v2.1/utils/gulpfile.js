...
function getFolders(dir) {
    // console.log('1st task: go into assets dir and ')
    return fs.readdirSync(dir)
        .filter(function(file) {
            // console.log(dir, file)
            // console.log(fs.statSync(path.join(dir, file)))
...
gulp.task('default', function() {
    ...
    // console.log("first task is called Default")
            ...
            if (parsed.ext === '.png'){
                if (parsed.name.endsWith("_2x")){
                    lineString = '#' + parsed.name + ' {\n' +
                                ...
                                '\tbackground-size:' + Math.floor(file.bbox.width/2) + 'px ' + Math.floor(file.bbox.height/2) + 'px;\n' +
                                '}\n';
