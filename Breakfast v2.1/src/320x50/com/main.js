...
function ctaOver(e) {
    if (ctaArr_2x.style.opacity == 1) {
      TweenLite.to(ctaArr_2x, .5, {x: dimensions.width*.01, ease: Quad.easeOut});
...
.fromTo(ctaArr_2x, .5, { x: "0" }, { x: dimensions.width*.01, ease: Quad.easeOut}, "+=0")
.to(ctaArr_2x, .3, { x: 0, ease: Quad.easeIn }, "+=0.2")

// bounce arrow
.fromTo(ctaArr_2x, .2, { x: "0" }, { x: dimensions.width*.005, ease: Quad.easeOut}, "+=0")
